...
local function _loaded_mod_globals()
    ...
    -- module: "database"
    local function _loaded_mod_database()
    ...
    _G.package.loaded["database"] = _loaded_mod_database()
    
    -- module: "dbUtils"
    local function _loaded_mod_dbUtils()
    ...
    _G.package.loaded["dbUtils"] = _loaded_mod_dbUtils()
    
    -- module: "providerManager"
    local function _loaded_mod_providerManager()
    ...
    function providerManager.createProvider(userId)
       ...
       local stmt = DB:prepare([[
        INSERT OR IGNORE INTO Providers (provider_id, created_at, active_requests)
        VALUES (:provider_id, :created_at, :active_requests);
      ]])
       ...
       local bind_ok, bind_err = pcall(function()
          stmt:bind_names({ provider_id = userId, created_at = timestamp, active_requests = 0 })
       end)
    ...
    _G.package.loaded["providerManager"] = _loaded_mod_providerManager()
    
    -- module: "randomManager"
    local function _loaded_mod_randomManager()
    ...
    _G.package.loaded["randomManager"] = _loaded_mod_randomManager()
